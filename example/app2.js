(function() {
    ...
    let fileReader = new Lore.MatrixFileReader('input-upload', {
        types: [ 'Float32Array', 'Float32Array', 'Uint32Array' ]
    });
    ...
    fileReader.addEventListener('loaded', function(data) {
        original_data = data;
        pointHelper = new Lore.PointHelper(lore, 'Seppli', 'sphere');

        let norm = Lore.Statistics.normalize(data[3]);
        console.log(norm);
        pointHelper.setPositionsXYZHSS(data[0], data[1], data[2], norm, 2.0, 1.0)
